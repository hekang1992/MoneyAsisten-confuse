...
class PolygonIdlPersonalView: UIView,UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        rowsetView.image = UIImage(named: "cilIconAbcd")
        ...
        sealedLabel.text = "Informasi Pekerjaan"
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        }else if paceBrick == "vry" {//城市选择
            self.nabobshipKnapsackEquivalence(oamArray,cell!,model)
        }else {}
    }
    ...
    func nabobshipKnapsackEquivalence(_ array: [IncomesModel], _ cell: CodeRegularCommonCell, _ model: CravedModel) {
        let eaglestoneView = YabberMethodCityView()
        eaglestoneView.frame = self.bounds
        eaglestoneView.array = array
        let vaaljapieAlert = TYAlertController(alert: eaglestoneView, preferredStyle: .actionSheet)
        fabricationLabialFabricant()?.present(vaaljapieAlert!, animated: true)
        eaglestoneView.block1 = {
            fabricationLabialFabricant()?.dismiss(animated: true)
        }
        eaglestoneView.block2 = { wackeStr in
            fabricationLabialFabricant()?.dismiss(animated: true,completion: {
                cell.textField1.text = wackeStr
                cell.model.saveStr = wackeStr
            })
        }
    }
